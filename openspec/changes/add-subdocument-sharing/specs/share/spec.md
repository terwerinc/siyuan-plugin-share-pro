...
系统 SHALL 使用SQL查询实现子文档的递归获取逻辑，基于生产验证过的SQL方案。该方案通过查询blocks表的path字段来识别子文档，完全兼容思源笔记的数据结构。

#### Scenario: 获取子文档总数
- **WHEN** 调用 getSubdocCount 方法时
- **THEN** 系统 SHALL 执行以下SQL查询：
```sql
SELECT COUNT(DISTINCT b1.root_id) AS count
FROM blocks b1
WHERE b1.root_id = '[docId]' OR b1.path LIKE '%/[docId]%'
```
- **THEN** 系统 SHALL 返回子文档总数

#### Scenario: 分页获取子文档列表
- **WHEN** 调用 getSubdocsPaged 方法时
- **THEN** 系统 SHALL 执行以下SQL查询：
```sql
...
```
- **THEN** 系统 SHALL 支持分页参数（page, pagesize）以避免内存溢出
- **THEN** 返回的文档列表 SHALL 包含 root_id、content、path、updated、created 等完整信息
...
#### Scenario: 性能优化
- **WHEN** 获取子文档列表时
- **THEN** 系统 SHALL 使用广度优先遍历（BFS）算法（用户确认偏好）
- **THEN** 子文档信息 SHALL 缓存5分钟（比原提案的1分钟更长，适合大型文档树）
...
#### Scenario: 与增量分享协同
- **WHEN** 用户使用增量分享功能时
- **THEN** 系统 SHALL 支持作为增量分享的扩展功能
- **THEN** 系统 SHALL 也支持独立的单次分享功能
- **THEN** 子文档分享设置 SHALL 自动继承
- **THEN** 增量检测 SHALL 包含所有子文档
- **THEN** 黑名单过滤优先级 SHALL 最高
- **THEN** 子文档数量和深度限制 SHALL 在增量分享中同样生效
