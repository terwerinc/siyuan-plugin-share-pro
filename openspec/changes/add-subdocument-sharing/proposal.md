...
1. **数量限制**：
   - 默认限制：100个子文档
   - 最大限制：999个子文档
   - 无限制选项：提供"无限制"选项，但必须明确提示性能风险
   - 风险提示：超过999个子文档时显示明确警告，告知可能的性能影响

2. **扁平化处理**：
   - 移除深度控制，采用完全扁平化的子文档获取方式
   - 获取文档下的所有子文档，无论其在文档树中的深度层级
   - 用户只需关心要分享多少个子文档，无需考虑深度限制
   - 简化用户体验，避免深度和数量的双重控制复杂性

3. **性能优化**：
   - 使用广度优先遍历（BFS）算法获取子文档（用户确认偏好）
   - 子文档信息缓存5分钟（比原提案的1分钟更长，适合大型文档树）
   - 支持异步加载子文档树，不阻塞UI主线程
   - 实现虚拟滚动，大量子文档时也保持流畅
