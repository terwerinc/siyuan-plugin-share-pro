...
- **THEN** 系统 SHALL 移除用户可配置的深度设置
- **THEN** 超过深度限制的引用 SHALL 不再继续分享并记录日志
- **THEN** 在分享结果中 SHALL 显示被截断的引用信息
...
#### Scenario: 性能限制
- **WHEN** 引用文档数量较多时
- **THEN** 系统 SHALL 自动优化处理，无需用户分批分享
- **THEN** 系统 SHALL 提供“仅分享首层引用”快捷选项
- **THEN** 系统 SHALL 显示预估时间和存储占用信息
...
#### Scenario: 文案翻译
- **WHEN** 系统显示引用文档相关文案时
- **THEN** 所有文案 SHALL 使用现有 cs.xxx 命名空间（保持简洁）
- **THEN** 系统 SHALL 支持中文和英文
- **THEN** 关键文案 SHALL 包括：shareReferences、shareReferencesEnabled、shareReferencesDisabled
