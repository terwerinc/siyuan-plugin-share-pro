...
**更新** 本版本增加了对 Vitest 测试框架的深度集成说明，包括测试策略、配置改进和 zhi-siyuan-api 依赖版本升级到 2.35.1 的相关信息。同时新增了 pnpm 工作区配置的详细说明，体现现代包管理最佳实践。
...
**更新** 服务层现已深度集成 Vitest 测试框架，支持增量分享服务的单元测试和集成测试。
...
participant Test as "Vitest测试框架"
App->>Plugin : 加载插件
...
**更新** 服务层现已集成 Vitest 测试框架，支持增量分享服务的全面测试覆盖。
...
**更新** 增量分享服务现已成为 Vitest 测试框架的重点测试对象，包含完整的单元测试和集成测试策略。
...
## 依赖分析
- 包管理器：pnpm（版本在 package.json 中声明，当前使用 pnpm@10.32.1）。
...
- 运行时依赖：Svelte 5、siyuan 插件运行时、zhi-* 生态库、虚拟列表等。
- **测试框架**：Vitest 4.1.1（单元测试与集成测试），支持监视模式开发。

**更新** 依赖版本更新：zhi-siyuan-api 升级至 2.35.1 版本，提供更好的 API 兼容性和稳定性。
...
**更新** 性能优化新增：增量检测缓存机制（5分钟有效期）和 Vitest 监视模式支持。
...
## 结论
本指南提供了从环境搭建到发布运维的完整开发路径，结合现有配置与代码结构，开发者可以快速上手并保持高质量交付。**最新版本**进一步强化了测试驱动开发能力，通过 Vitest 框架实现了全面的单元测试和集成测试覆盖，同时升级了 zhi-siyuan-api 依赖以获得更好的兼容性和稳定性。新增的 pnpm 工作区配置体现了现代包管理的最佳实践，为项目的长期发展奠定了坚实基础。建议在团队内统一遵循本文档的规范与流程，持续完善 CI/CD 与测试策略。

## 附录

### 开发环境搭建与依赖安装
- 使用包管理器：pnpm（版本在 package.json 中声明，当前使用 pnpm@10.32.1）。
- 安装依赖：在项目根目录执行安装命令。
...
**更新** 新增 Vitest 测试框架依赖说明和 pnpm 工作区配置说明。
...
**更新** 测试脚本已集成到 package.json，支持监视模式开发。
...
- 问题：**pnpm 工作区配置问题**
  - 解决：检查 pnpm-workspace.yaml 配置，确保 esbuild 构建支持正确设置。
- 最佳实践：保持服务层与 UI 层解耦，使用互斥锁避免重复请求，合理拆分工具函数与模型，**为每个核心功能编写对应的测试用例**。

**更新** 新增测试相关常见问题和最佳实践，以及 pnpm 工作区配置问题解决方案。
...
- 步骤建议：
  ...
  - 生成变更日志与版本同步。
  - 上传制品（可选）。
- 注意事项：确保 CI 环境变量与本地一致，避免因权限或网络导致失败，**测试阶段使用 Vitest 进行自动化测试**。

**更新** 在 CI/CD 流程中增加 Vitest 测试步骤。
