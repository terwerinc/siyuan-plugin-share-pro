...
**更新** 本版本增加了对 Vitest 测试框架的深度集成说明，包括测试策略、配置改进和 zhi-siyuan-api 依赖版本升级到 2.33.0 的相关信息。
...
- **测试框架**：Vitest 4.0.15（单元测试与集成测试），支持监视模式开发。

**更新** 依赖版本更新：zhi-siyuan-api 升级至 2.33.0 版本，提供更好的 API 兼容性和稳定性。
...
## 结论
本指南提供了从环境搭建到发布运维的完整开发路径，结合现有配置与代码结构，开发者可以快速上手并保持高质量交付。**最新版本**进一步强化了测试驱动开发能力，通过 Vitest 框架实现了全面的单元测试和集成测试覆盖，同时升级了 zhi-siyuan-api 依赖以获得更好的兼容性和稳定性。建议在团队内统一遵循本文档的规范与流程，持续完善 CI/CD 与测试策略。
...
- **测试依赖**：Vitest 4.0.15 已集成，支持监视模式开发。

**更新** 新增 Vitest 测试框架依赖说明。
