...
- [src/libs/components/bench/BenchSummary.svelte](file://src/libs/components/bench/BenchSummary.svelte)
...
## 更新摘要
**所做更改**
- 新增可配置引用文档分享深度限制功能，采用智能默认深度（3层）
- 增强手动子文档选择能力，支持用户自定义子文档选择列表
- 改进UI可访问性支持，增加键盘导航和屏幕阅读器支持
- 优化引用文档分享的性能保护和循环引用检测机制
...
**更新** 新增了可配置引用文档分享深度限制功能、增强手动子文档选择能力和改进UI可访问性支持。
...
CONST["Constants<br/>src/Constants.ts"]
end
subgraph "组合式工具"
...
STATUS["statusBar<br/>src/statusBar.ts"]
end
subgraph "UI组件与可访问性"
BP["BenchPager<br/>src/libs/components/bench/BenchPager.svelte"]
BS["BenchSummary<br/>src/libs/components/bench/BenchSummary.svelte"]
CONFIRM["Confirm<br/>src/libs/components/Confirm.svelte"]
end
subgraph "类型定义"
...
- [src/libs/components/bench/BenchPager.svelte:80-154](file://src/libs/components/bench/BenchPager.svelte#L80-L154)
- [src/libs/components/bench/BenchSummary.svelte:39-47](file://src/libs/components/bench/BenchSummary.svelte#L39-L47)
- [src/libs/components/Confirm.svelte:81](file://src/libs/components/Confirm.svelte#L81)
...
- Constants：包含 DEFAULT_SHARE_REFERENCES_MAX_DEPTH 常量，控制引用文档分享的最大递归深度。
- UI组件：BenchPager、BenchSummary、Confirm 等组件提供键盘导航和屏幕阅读器支持。
- 类型定义：service-dto.d.ts 与 types/index.d.ts 提供服务端 DTO 与统一导出。

**更新** 新增 Constants 模块提供引用文档分享深度限制配置，UI组件增强可访问性支持。
...
- 职责概览
  ...
  - **新增** 引用文档深度限制：使用 DEFAULT_SHARE_REFERENCES_MAX_DEPTH 常量控制递归深度
  - **新增** 手动子文档选择：支持用户自定义子文档选择列表
- 关键方法与流程
...
**更新** 新增引用文档深度限制和手动子文档选择功能，增强了系统的智能化程度和用户体验。
...
+collectSubdocuments(...) Promise~Array~
+collectReferencedDocuments(...) Promise~Array~
-addLog(msg, type) void
}
class Constants {
<<static>>
DEFAULT_SHARE_REFERENCES_MAX_DEPTH : number
}
...
ShareService --> Constants : "使用深度限制"
ShareService --> SingleDocSetting : "支持手动选择"
```
...
- 功能要点
  ...
  - **更新** 子文档：支持手动选择和自动获取两种模式，分页拉取并过滤根文档自身，应用数量限制
  - **新增** 引用文档：递归查询 refs 表，最大深度默认 3 层，循环引用检测
  - 去重合并，返回扁平化列表
...
C -- 否 --> D["仅返回主文档"]
C -- 是 --> E["检查手动选择"]
E --> F{"selectedSubdocIds 是数组?"}
F -- 是 --> G["使用用户选择的子文档"]
F -- 否 --> H["自动获取所有子文档"]
H --> I["计算最大数量(含无限制)"]
I --> J["分页拉取子文档(排除根)"]
J --> K["收集子文档"]
K --> L["递归获取引用文档(深度<=3, 去重)"]
G --> M["合并去重后的文档列表"]
L --> N["合并去重后的文档列表"]
D --> O["输出"]
N --> O
...
### 引用文档分享（递归收集）
- **更新** 深度限制：使用 DEFAULT_SHARE_REFERENCES_MAX_DEPTH 常量，默认最大 3 层
...
### 手动子文档选择
- **新增** 场景区分：
  - ShareUI 场景：如果 settings.selectedSubdocIds 是数组，直接使用用户选择的文档ID
  - 其他场景：selectedSubdocIds 为 undefined，自动获取所有子文档
- 优先级：手动选择优先级高于自动获取，即使是空数组也尊重用户选择
- 过滤规则：排除主文档本身和已添加的文档
...
- 组件耦合
  ...
  - **新增** 与 Constants 的耦合，用于引用文档分享深度限制
- 外部依赖
...
- **更新** 引用文档深度限制：默认最大递归深度 3 层，防止无限递归导致的性能问题
- **更新** 循环引用检测：内置循环引用检测，避免重复处理和死循环
...
## 结论
ShareService 通过清晰的职责划分与稳健的流程设计，实现了从单文档到多文档、从内容提取到资源处理的完整分享闭环。其扁平化与递归收集机制兼顾灵活性与性能，配合进度管理与错误处理，为用户提供了可靠的分享体验。**更新** 新增的引用文档深度限制、手动子文档选择和UI可访问性改进进一步提升了系统的智能化程度和用户体验。
...
- createShare(docId, settings?, options?)
  - 参数
    ...
    - settings?: Partial<SingleDocSetting>，可选，文档级设置（如是否显示文档树/大纲、分享有效期、是否分享子文档/引用文档、子文档数量限制、**新增**手动选择的子文档ID列表等）
...
**更新** 新增对 SingleDocSetting 中 selectedSubdocIds 字段的支持，允许用户手动指定要分享的子文档列表。
...
- [src/models/SingleDocSetting.ts:81-88](file://src/models/SingleDocSetting.ts#L81-L88)
- [src/api/share-api.ts:46-50](file://src/api/share-api.ts#L46-L50)
...
### UI可访问性改进
- **键盘导航支持**：BenchPager 组件增加了 tabindex 属性，支持键盘导航
- **屏幕阅读器支持**：BenchSummary 组件使用 role="status" 和 aria-live="polite" 属性
- **标签描述**：Confirm 组件增加了 aria-label 属性，提供更好的语义化描述
- **状态栏可访问性**：statusBar 组件增加了 aria-label 属性，改善用户体验
